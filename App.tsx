...
import { Search, Menu, X, ArrowLeft, FileText, ChevronRight } from 'lucide-react';
import { Prescription } from './types';

function App() {
  const [viewMode, setViewMode] = useState<'specialty' | 'alphabetical'>('specialty');
  const [selectedSpecialtyId, setSelectedSpecialtyId] = useState<string | null>(specialties[0].id);
  ...
  const handleSelectSpecialty = (id: string) => {
    ...
    setSelectedPrescription(null); // Back to list
    setSearchTerm('');
      ...
      <div className="flex-1 flex flex-col h-full w-full overflow-hidden relative">
              ...
              <input 
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value && selectedPrescription) {
                    setSelectedPrescription(null); // Exit detail view on search
                  }
                }}
              />
            </div>
        ...
        <main className="flex-1 overflow-y-auto custom-scroll bg-[#f3f4f6] p-4 md:p-8">
          <div className="max-w-4xl mx-auto">
            ...
            {selectedPrescription ? (
              ...
              <div className="animate-in fade-in duration-300">
                <div className="flex items-center justify-between mb-6 border-b-2 border-medical-primary/10 pb-4">
                  <h2 className="text-2xl md:text-3xl font-serif text-medical-primary font-bold">
                    {currentSpecialtyName}
                  </h2>
                  <span className="bg-blue-100 text-blue-800 text-xs px-3 py-1 rounded-full font-mono">
            ...
            {/* Footer */}
            <div className="mt-12 mb-4 text-center text-xs text-gray-400 print:hidden">
              MediScript v2.1
            </div>
          </div>
        </main>
